...
| Source/API | Operation or observable contract | Ordering | Allocation | Termination/error behavior | Test evidence | Status/notes |
|---|---|---|---|---|---|---|
| `active_motion_set` | Sparse registration, update, stationary removal, deactivation, clear, legacy per-record batch application, and workspace-backed sort/coalesce/merge batches | Ascending runtime `NodeHandle`, independent of distinct-node registration order; duplicate batch updates are last-write-wins | Set and batch workspace vectors grow on demand and retain capacity; storage and scratch capacity are observable | Every handle is validated before optimized batch mutation; stationary state removes rather than disables a record; invalid and absent-deactivation batches preserve records and generation; repeated clear is a no-op | `scene_polytree.motion` registration, ordering, atomic batch, and lifecycle case | **Current** |
...
| Generic articulation benchmark fixture | A root, yaw pivot, and pitch pivot exercise the real rigid-pose policy | Central motion and transform ordering | Benchmark-private support only; no installed domain API | Stationary commands empty active storage and moving commands exercise partial propagation | `scene_polytree_performance_benchmark` smoke matrix | **Current measurement fixture** |
